        ...
        string messageId)
    {
        var verifyServiceBusMessage = await serviceBusListenerMock
            .When(
                message =>
                {
                    if (message.Subject != EnqueueActorMessagesV1.BuildServiceBusMessageSubject(Brs_021_SendMeasurements.V1))
                        return false;

                    var body = EnqueueActorMessagesV1
                        .Parser.ParseJson(message.Body.ToString())!;

                    var forwardMeteredDataAcceptedV1 = JsonSerializer.Deserialize<SendMeasurementsAcceptedV1>(body.Data);
